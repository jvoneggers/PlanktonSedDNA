...
with linear models and t-tests by lake - no residual check
```{r}
# combine all three of the NMDS data frames
nmds_comb <-rbind(read.csv("/Users/jordanscheibe/Library/CloudStorage/OneDrive-UniversityofWyoming/LakeSedDNA/Data/PhytoZoopSedDNA/2_DataAnalysis/NMDS_phyto.csv", header=T, row.names = 1) %>% 
                   select(sample_id, MDS2,d15N_anomaly_mean_1800_1850, mean_air_temp_degC_anomaly_combined, fish_present_binary_YN) %>% 
...
# Function to compute p-value for each group
compute_pval <- function(data) {
  model <- lm(NMDS ~ mean_air_temp_degC_anomaly_combined, data = data)
  summary(model)$coefficients[2, 4]  # Extract p-value for slope
}

# Compute p-values per trophic level, lake, and state
group_pvals <- nmds_comb %>%
  group_by(trophic_level, lake_name, state) %>%
  summarise(pval = compute_pval(pick(everything())), .groups = "drop")

# Filter significant groups
significant_groups <- group_pvals %>%
  filter(pval < 0.05) %>%
  select(trophic_level, lake_name, state)  # Keep all grouping vars

# Merge with original data to filter for significant groups only
filtered_data <- nmds_comb %>%
  inner_join(significant_groups, by = c("trophic_level", "lake_name", "state"))

# Plot
temp <- ggplot(nmds_comb, aes(x = mean_air_temp_degC_anomaly_combined, 
                              y = NMDS, color = lake_name)) +
  facet_grid(trophic_level ~ state) +  # Facet by trophic level & state
  geom_point() +
  geom_smooth(
    data = filtered_data, 
    method = "lm",
...
# Function to compute p-value for each group
compute_pval <- function(data) {
  model <- lm(NMDS ~ d15N_anomaly_mean_1800_1850, data = data)
  summary(model)$coefficients[2, 4]  # Extract p-value for slope
}
...
# Plot
nit <- ggplot(nmds_comb, aes(x = d15N_anomaly_mean_1800_1850, 
                              y = NMDS, color = lake_name)) +
  facet_grid(trophic_level ~ state) +  # Facet by trophic level & state
  geom_point() +
  geom_smooth(
...
ggsave("3_Figures/SFigX_NMDS_state_stressor_trophiclevel_lake.png", height=6, width=16, units="in")
```
...
ggsave("3_Figures/FigX_NMDS_natural_variability.png", height=6.5, width=15, units="in")

...
# select only phototrophs and mixotrophs, group by taxonomic rank and turn to relative abundance
sub <- subset_taxa(ps_V7_norm, trophic_mode %in%c("phototrophs","phototrophs/mixotrophs","mixotrophs")) %>% tax_glom(.,taxrank="subdivision") %>% transform_sample_counts(., function(x) x / sum(x)) %>% psmelt(.)
...
sub$lake_name <- factor(sub$lake_name, levels = names(lake_colors))
...
ggsave("3_Figures/FigX_relative_abundance_V7_top_phytoplankton.png",height=10, width=9, units="in")
...
```{r}
# run mixed effects models over prism time frame
lme_data <-sub %>% filter(is.na(mean_air_temp_degC_interval)==F & is.na(d15N_diff_from_start)==F) %>% select(Abundance,mean_air_temp_degC_interval,d15N_diff_from_start,fish_present_binary,lake_name,subdivision, mid_cm) %>%   
        arrange(lake_name,subdivision,desc(mid_cm)) %>% group_by(lake_name) %>% mutate(time_seq=row_number()) %>% ungroup()
...
# extract model estimates and put into a table for each iteration
      ...
      model_table <- as_tibble(model_summary$tTable, rownames = "term")
      model_table <- model_table %>% mutate(taxa = i) %>% rename(p.value='p-value')
...
ggsave("3_Figures/FigX_lme_V7_top_phytoplankton.png",height=4, width=7, units="in")


write.csv(as.data.frame(model_output), "2_DataAnalysis/LME_phytoplankton_model_output.csv")
...
ggsave("3_Figures/FigX_NMDS_heterotrophic_protists_V7_ESV.png", width = 7, height = 5, dpi = 400)
```
...
# select only phototrophs and mixotrophs, group by taxonomic rank and turn to relative abundance
sub <- subset_taxa(ps_V7_norm, !trophic_mode %in%c("phototrophs","phototrophs/mixotrophs","mixotrophs") &  !subdivision=="Metazoa") %>% tax_glom(.,taxrank="subdivision") %>% transform_sample_counts(., function(x) x / sum(x)) %>% psmelt(.)
...
ggsave("3_Figures/FigX_relative_abundance_V7_top_heterotrophic_protists.png",height=10, width=9, units="in")
...
ggsave("3_Figures/FigX_lme_V7_top_heterotrophic_protists.png",height=4, width=7, units="in")

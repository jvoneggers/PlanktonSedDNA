...
lme_data <-sub %>% filter(is.na(d15N_anomaly_mean_1800_1850)==F) %>%
        ...
        ungroup() %>% mutate(d15N_anomaly_mean_1800_1850=(d15N_anomaly_mean_1800_1850*(-1)))
...
ggsave("3_Figures/FigX_LME_summary_fish.png", height=5, width=7, units="in")
```
...
tax_order<-tax_order[c(7,8,9,10,1,2,3,4,5,6,11,12,13,14,15,16,17,18)]
all_trophic$trophic_level_tax_group<-factor(all_trophic$trophic_level_tax_group, levels=tax_order)
rm(tax_order)

# subset by taxa with significant LME results
...
clust_tax<-rbind(
        ...
        read.csv("/Users/jordanscheibe/Library/CloudStorage/OneDrive-UniversityofWyoming/LakeSedDNA/Data/PhytoZoopSedDNA/2_DataAnalysis/V9_LME_zooplankton_model_output_no_interactions.csv", header=T, row.names=1) %>% filter(pval0.5=="Yes") %>% mutate(trophic_level="Zooplankton")) %>% select(taxa) %>% distinct(.) %>% pull()
        
all_trophic <- all_trophic %>% filter(taxonomic_group %in% clust_tax)

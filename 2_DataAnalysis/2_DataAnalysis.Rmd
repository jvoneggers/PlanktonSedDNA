...
# calculate the difference from starting value for d15N WITHOUT the CNS adjusted value
d15N_start_summary <- pb %>% group_by(lake_name) %>%
        filter(is.na(d15N)==F) %>%
...
### written by chatGPT, just to have a look
```{r}
library(broom)
...
clust_tax<-rbind(
        ...
        read.csv("/Users/jordanscheibe/Library/CloudStorage/OneDrive-UniversityofWyoming/LakeSedDNA/Data/PhytoZoopSedDNA/2_DataAnalysis/V9_LME_zooplankton_model_output_no_interactions.csv", header=T, row.names=1) %>% filter(pval0.5=="Yes") %>% mutate(trophic_level="Zooplankton")) %>% 
        # select taxa that have an effect greater than 0.3
...
# the code above doesn't include prior to 1400


all_trophic %>% filter(trophic_level_tax_group=="Phytoplankton - Chlorophyta" & date_mid_AD_combo <1400) %>% pull(abundance) %>% mean(.)
...
ggsave("3_Figures/FigX_clustered_rel_abund_lake_fish_avg_50yrs.png", height=6.5, width=7, units="in")
...
summary_effects<-rbind(
        ...
        filter(Value > 0.04 | Value < (-0.04)) # %>% group_by(taxa) %>%

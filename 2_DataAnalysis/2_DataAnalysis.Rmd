...
model_stats <- nmds_comb %>%
  ...
  mutate(
    p_value = sapply(lm_fit, function(m) summary(m)$coefficients[2, 4]), # p-val
    r_squared = sapply(lm_fit, function(m) summary(m)$r.squared),
    residuals = lapply(lm_fit, residuals), # grab residuals
    shapiro_p = sapply(residuals, function(res) shapiro.test(res)$p.value), # test for normality
    bp_p = sapply(lm_fit, function(m) bptest(m)$p.value) # test for heteroscedasticity
  ) %>%
  filter(p_value < 0.05) %>% 
  mutate(
    label = paste0("R² = ", signif(r_squared, 3)),
...
ggsave("3_Figures/FigX_NMDS_state_stressor_trophiclevel_res_check.png", height=6, width=16, units="in")
```




with linear models and t-tests by lake
```{r}
# combine all three of the NMDS data frames
nmds_comb <-rbind(read.csv("/Users/jordanscheibe/Library/CloudStorage/OneDrive-UniversityofWyoming/LakeSedDNA/Data/PhytoZoopSedDNA/2_DataAnalysis/NMDS_phyto.csv", header=T, row.names = 1) %>% 
                   select(sample_id, MDS2,d15N_anomaly_mean_1800_1850, mean_air_temp_degC_anomaly_combined, fish_present_binary_YN) %>% 
...
#### i. Stats for ms - avgs overall - not updated only looking after 1600 but stats should be the same, but this does remove before 1600 so not the same - needs updating!
##### L1 - phytoplankton
```{r}

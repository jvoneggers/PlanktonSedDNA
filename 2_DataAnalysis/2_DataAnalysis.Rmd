...
# Load packages, custom theme, color, and data
```{r}
...
# anomaly
ggplot(data = meta, aes(x = date_mid_AD_combo, y = d15N_diff_from_start)) +
  geom_line() + 
  geom_point() + 
  ...
  scale_y_reverse()+
  custom_theme()  
ggsave("3_Figures/FigX_d15N_diff_from_start.png",height=5, width=16, units="in")
...
breakpoint_index <- c("Canyon" = 17, "Eyrie" = 11, "Lightning" = 13, 
                 "Black Rock" = 7, "Footprint" = 13, "Lost" = 15, "Scott" = 11, 
                 "Bullfrog"= 16, "Middle Gaylor"= 16, "Soldier"=16, "Skelton"=19, 
                 "Crescent"= 14, "Louise"=22,"Monogram"=31)
...
# try individually by lake

# 
# model_output<-as_tibble(NULL)
# for(l in unique(lme_data$lake_name)){
# for(i in unique(lme_data$subdivision)){
#         model <- lm(
#                 Abundance ~ mean_air_temp_degC_interval + d15N_diff_from_start + fish_present_binary,
#                 data =lme_data %>% filter(subdivision == i & lake_name == l))
# # extract model estimates and put into a table for each iteration
#             model_summary <- summary(model)
#             model_table <- as_tibble(coef(model_summary), rownames = "term")
#             model_table <- model_table %>% mutate(taxa = i, lake_name = l) %>% rename(p.value = 'Pr(>|t|)')
#             model_output <- bind_rows(model_output, model_table)
#       }} ; rm(model); rm(i); rm(l); rm(model_table)
# 
# 
#             
# model_output<-model_output %>%
...
#         mutate(term = case_when(term=="(Intercept)"~"Intercept",
#                                 term=="mean_air_temp_degC_interval"~"Mean annual air\ntemperature (deg C)",
#                                 term=="d15N_combined"~"d15N",
#                                 term=="fish_present_sample"~"Fish presence"),
#         term = factor(term, levels = c("Intercept","Mean annual air\ntemperature (deg C)","d15N", "Fish presence"))) %>% filter(!term=="Intercept")
...
### set up one file with all abundances of all of the three taxa groups
...
effect_summary <- model_output %>% filter(p.value>0.6) %>%
...
effect_summary_lake<-left_join(complete_rows, effect_summary, join_by(lake_name,trophic_level,term)) %>% 
...
#rm(additional_rows,gls_data,model_output,subset,sub,ps_V7_norm_hetero,ps_V7_norm_phyto,ps_V7_norm_zoop,tmp,ps_V7_norm_sub,all_trophic)
...
ggsave("3_Figures/FigX_gls_V7_lake.png", width = 12, height = 5, dpi = 400)
...
```{r}
# Summarize mean effect for each term by taxonomic group and lake
effect_summary_state <- model_output %>% filter(p.value>0.6) %>%
...
(p3 | p4) +   plot_layout(guides = "collect",axis_titles = "collect") + 
  plot_annotation(tag_levels = 'A')

ggsave("3_Figures/FigX_gls_V7_state.png", width = 10, height = 5, dpi = 400)
```
...
```{r}
# Summarize mean effect for each term by taxonomic group and lake
effect_summary_total<- model_output %>% filter(p.value>0.6) %>%
  ...
  arrange(trophic_level, term) %>% mutate(total="Total")
...
ggsave("3_Figures/FigX_gls_V7_total.png", width =10, height = 5, dpi = 400)
```


```{r}
(p1| p3 | p5 ) + plot_layout(guides = "collect", axis_titles = "collect") + 
   plot_annotation(tag_levels = 'A')

ggsave("3_Figures/FigX_V7M.png", width =10, height = 5, dpi = 400)
```
...
require(forecast)
test<-lme_data%>% filter(subdivision == unique(lme_data$subdivision)[1] & lake_name=="Middle Gaylor")  
test<-ts(test$Abundance,start=1,end=length(test$Abundance),frequency = 1)
plot(test)
fit<-auto.arima(test,stationary = F, seasonal=F, trace=T)
arimax()

library(forecast)
# Suppose `predictor1` and `predictor2` are external variables you suspect may affect `response`
model_arimax <- auto.arima(test$Abundance, xreg = cbind(test$mean_air_temp_degC_interval, test$d15N_diff_from_start))

summary(model_arimax)
model_arima <- auto.arima(response)  # ARIMA without predictors
model_arimax <- auto.arima(response, xreg = cbind(predictor1, predictor2))  # ARIMAX with predictors
...
# Compare AIC/BIC
AIC(fit, model_arimax)
BIC(model_arima, model_arimax)

library(lmtest)
grangertest(Abundance ~ d15N_diff_from_start + mean_air_temp_degC_interval+ fish_present_binary, order = 1, data = test)

install.packages("vars")
library(vars)
# Fit a VAR model with the predictors and response
var_model <- VAR(test[, c("Abundance", "d15N_diff_from_start", "mean_air_temp_degC_interval", "fish_present_binary")], p = 1)
summary(var_model)
...
test<-lme_data%>% filter(subdivision == unique(lme_data$subdivision)[1] & lake_name=="Footprint")  

data <- data.frame(abundance_diff = diff(test$Abundance), 
                       nitrogen_diff = diff(test$d15N_diff_from_start),
                       climate_diff = diff(test$mean_air_temp_degC_interval),
                       fish=as.factor(test$fish_present_binary[-(length(test$fish_present_binary)-1)])) # is this right to remove one of the observations? it removes the first i believe.. which seems to be correct from difference.

library(mgcv)

# Fit a model where `s` represents smooth terms for continuous predictors
model <- gam(abundance_diff ~ s(climate_diff) + s(nitrogen_diff) + fish, data = data)
model <- gam(abundance_diff ~ climate_diff + nitrogen_diff + fish, data = data)


# Check model summary
summary(model)

model<-gam(Abundance~mean_air_temp_degC_interval + d15N_diff_from_start + fish_present_binary,
                data =test)
summary(model)


plot(test$time_seq, test$Abundance, type="l")
plot(test$time_seq, test$mean_air_temp_degC_interval,  col="green", type="l")
plot(test$time_seq, test$d15N_diff_from_start, col="blue", type="l")
...
model<-gls(Abundance~mean_air_temp_degC_interval + d15N_diff_from_start + fish_present_binary,
                data =test, correlation = corAR1(form = ~ time_seq))
<-summary(model)

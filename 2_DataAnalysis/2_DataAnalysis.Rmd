...
thoughts - for the modelling, perhaps I don't want to make it look positive for d15N, leave it as normal, reverse the axes and then model with the anomaly values
 ...
 geom_polygon(data = poly, aes(x = x, y = y, fill = lake_name), alpha = 0.5)+
  ...
  facet_wrap2(
    ...
    strip = strip_themed(
      ...
      text_x = elem_list_text(color = ifelse(unique(lk$lake_name)%in% c("Lightning","Lost","Scott","Skelton","Soldier"), "white", "black"))
    )
  )+
...
ggplot(data=metadata, aes(x=date_mid_AD_combo,y=mean_air_temp_degC_anomaly_combined))+
    ...
    strip = strip_themed(
      ...
      text_x = elem_list_text(
        color = ifelse(names(lake_colors_2) %in% c("Lightning","Lost","Scott","Skelton","Soldier"), "white", "black"))))+
        geom_line(col="white") + 
...
ndep_niso<-read.csv("/Users/jordanscheibe/Library/CloudStorage/OneDrive-UniversityofWyoming/LakeSedDNA/Data/PhytoZoopSedDNA/2025-01-16_sample_data_dates.csv")  
...
# 8. Relative abundance, NMDS, LMEs by trophic level
...
lme_data_fish <- lme_data %>% filter(lake_name %in% c("Louise", "Monogram","Black Rock","Footprint","Lost", "Bullfrog","Middle Gaylor"))
lme_data_fishless <- lme_data %>% filter(!lake_name %in% c("Louise", "Monogram","Black Rock","Footprint","Lost", "Bullfrog","Middle Gaylor"))
```
...
model_output<-model_output %>%
        ...
        mutate(term = case_when(term=="(Intercept)"~"Intercept",
                                ...
                                term=="d15N_anomaly_mean_1800_1850"~"d15N",
...
write.csv(as.data.frame(model_output), "2_DataAnalysis/V9_LME_phytoplankton_model_output_no_interactions.csv")
...
RA<-ggplot(data = sub, aes(x = date_mid_AD_combo, y = Abundance, color = subdivision)) +
  ...
  geom_vline(
    data = sub %>% filter(!is.na(first_stocked_yr) & !lake_name%in%c("Skelton","Soldier")), 
...
tax_table(ps_V9_norm_sub) <- tax_table(tax_fun) # replace taxonomy table in the phyloseq object
rm(tax_fun)
ps_V9_norm_sub<-subset_taxa(ps_V9_norm_sub, !trophic_mode %in% c("phototrophs","phototrophs/mixotrophs","mixotrophs", "unknown")& !class %in% c("Arthropoda","Unassigned","Nematoda","Craniata","Tardigrada","Annelida","Gastrotricha","Platyhelminthes","Porifera","Mollusca","Metazoa_X","Cnidaria","Bryozoa")) 
# remove phototrophic/mixotrophic organisms and keep only Rotifers from the metazoan subdivision
```
- currrently not using unknown, but was before
...
#### ii. select taxa
```{r}
taxa_colors<-c( "#F0E442" ,"#0072B2" ,"#CC79A7" ,"#999999","#D55E00")
...
write.csv(as.data.frame(model_output), "2_DataAnalysis/V9_LME_heterotrophic_protists_model_output_no_interactions.csv")
...
ggsave("3_Figures/FigX_V9_heterotrophic_protist_NMDS_RelAbun_LME_nounknown.png",height=10, width=20, units="in")
...
write.csv(as.data.frame(model_output), "2_DataAnalysis/V9_LME_zooplankton_model_output_no_interactions.csv")
...
nit<-ggplot(nmds_comb, aes(x=d15N_anomaly_mean_1800_1850,y=NMDS, color=lake_name))+
        facet_grid(trophic_level~state)+
        geom_point()+ 
        scale_color_manual(values=lake_colors) +
...
mutate(term=case_when(term=="d15N"~"Nitrogen deposition",
                      term=="Mean annual air temperature\nanomaly (deg C)"~"Air temperature",
                      term=="Fish presence"~"Fish"))
...
ggplot(sig_lme, aes(x = factor(term), y = mean_effect)) +
  ...
  facet_wrap2(~trophic_level,  strip = strip_themed(
      background_x = elem_list_rect(fill = c("#009E73","#56B4E9","#D55E00")))) +
  theme_minimal() +
  labs(
...
ggsave("3_Figures/FigX_LME_summary.png", height=5, width=7, units="in")
```


## c. clustered relative abundances
- maybe want to doublec check any changes other than taking out unknown for the heterotrophic protists and changing the heterotrophic protists threshold to 0.55 
```{r}
...
ps_V9_norm_hetero<-subset_taxa(ps_V9_norm_sub, !trophic_mode %in% c("phototrophs","phototrophs/mixotrophs","mixotrophs", "unknown")& !class %in% c("Arthropoda","Unassigned","Nematoda","Craniata","Tardigrada","Annelida","Gastrotricha","Platyhelminthes","Porifera","Mollusca","Metazoa_X","Cnidaria","Bryozoa")) 
...
lake_colors_fish <- c("Crescent" = "#63B8FF", 
                 "Canyon" = "#63B8FF", 
                 "Eyrie" = "#63B8FF",
                 "Lightning" = "#63B8FF",
                 "Soldier" = "#63B8FF", 
...
# order the trophic levels so phytoplankton comes first
tax_order<-unique(all_trophic$trophic_level_tax_group)
tax_order<-tax_order[c(6,7,8,9,1,2,3,4,5,10,11,12,13,14,15,16,17)]
all_trophic$trophic_level_tax_group<-factor(all_trophic$trophic_level_tax_group, levels=tax_order)
rm(tax_order)



# remove extra objects
rm(sub);rm(ps_V9_norm_hetero);rm(ps_V9_norm_phyto);rm(ps_V9_norm_sub); rm(ps_V9_norm_zoop)

```

plot abundances by lake name and by fish/fishless
```{r}
...
ggsave("3_Figures/FigX_clustered_rel_abund_lake.png", height=10, width=20, units="in")
...
ggsave("3_Figures/FigX_clustered_rel_abund_lake_fish.png", height=10, width=20, units="in")
...
ggsave("3_Figures/FigX_clustered_rel_abund_lake_avg_50yrs.png", height=10, width=20, units="in")



avg_lake_20yrs<-all_trophic %>%         
        mutate(date_window_20yrs=cut(date_mid_AD_combo,
                       breaks = c(seq(1400, 2021, by = 20), 2022), 
                        right = FALSE, 
                        labels = c(paste(seq(1400, 2000, by = 20), 
                                         seq(1419, 2019, by = 20), sep = "-"), 
                                   "2020-2021"))) %>% 
               group_by(trophic_level_tax_group, date_window_20yrs) %>% 
                summarize(avg_abund=mean(abundance),
                  median_year = median(as.numeric(unlist(strsplit(unique(as.character(date_window_20yrs)), "-")))))


ggplot(data=all_trophic, aes(x=date_mid_AD_combo, y=abundance, color=lake_name))+
        facet_wrap(~trophic_level_tax_group)+
        geom_line() + 
        geom_line(data=avg_lake_20yrs, aes(x=median_year, y=avg_abund, color=NULL), linewidth=2, color="black")+
        custom_theme()+
        labs(color="Lake",
             y="Relative abundance (proportion)",
             x="Date (Common Era)")+
...
ggsave("3_Figures/FigX_clustered_rel_abund_lake_avg_20yrs.png", height=10, width=20, units="in")
```
...
ggplot(data=all_trophic, aes(x=date_mid_AD_combo, y=abundance, color=lake_name_fish))+
        facet_wrap(~trophic_level_tax_group)+
        geom_line(alpha = 0.6) + 
        geom_line(data=avg_lake_50yrs %>% filter(fish==1), aes(x=median_year, y=avg_abund, color=NULL), linewidth=2, color="#EE6363")+
        geom_line(data=avg_lake_50yrs %>% filter(fish==0), aes(x=median_year, y=avg_abund, color=NULL), linewidth=2, color="#63B8FF")+
        custom_theme()+
        labs(color="Lake",
             y="Relative abundance (proportion)",
             x="Date (Common Era)")+
...
ggsave("3_Figures/FigX_clustered_rel_abund_lake_fish_avg_50yrs.png", height=10, width=20, units="in")
...
ggplot(data=all_trophic, aes(x=date_mid_AD_combo, y=abundance, color=lake_name))+
        facet_wrap(~trophic_level_tax_group)+
        geom_line(alpha = 0.6) + 
        geom_line(data=avg_lake_50yrs %>% filter(state=="Washington"), aes(x=median_year, y=avg_abund, color=NULL), linewidth=2, color="orange")+
        geom_line(data=avg_lake_50yrs %>% filter(state=="Wyoming"), aes(x=median_year, y=avg_abund, color=NULL), linewidth=2, color="#008080")+
        geom_line(data=avg_lake_50yrs %>% filter(state=="California"), aes(x=median_year, y=avg_abund, color=NULL), linewidth=2, color="#5D478B")+
        custom_theme()+
        labs(color="Lake",
             y="Relative abundance (proportion)",
             x="Date (Common Era)")+
...
ggsave("3_Figures/FigX_clustered_rel_abund_lake_state_avg_50yrs.png", height=10, width=20, units="in")

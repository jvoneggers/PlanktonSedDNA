...
ggplot(data = metadata, aes(x = date_mid_AD_combo, y = mean_air_temp_degC_anomaly_combined, color = lake_name)) +
...
ggplot(data = meta, aes(x = date_mid_AD_combo, y = d15N_anomaly_mean_1800_1850, color = lake_name)) +
    facet_wrap2(~ state, ncol = 3, strip.position = "top",
                strip = strip_themed(background_x = elem_list_rect(fill = "white"))) +
...
#6. Relative abundance, NMDS, LMEs by trophic level
...
geom_vline(
    data = sub %>% filter(!is.na(first_stocked_yr) & lake_name%in%c("Skelton","Soldier")), 
    aes(xintercept = first_stocked_yr, linetype = "Fish present 30 years or less"), 
 ...
 scale_linetype_manual(
    values = c("N isotope breakpoint" = "dotted", "Year fish first stocked" = "solid", "Fish present 30 years or less" = "solid", "Temperature breakpoint" = "dashed"),
    breaks = c("N isotope breakpoint", "Year fish first stocked","Fish present 30 years or less", "Temperature breakpoint")) +
        guides(
...
taxa_colors<-c("gray50", "#E69F00" ,"#56B4E9" ,"#F0E442","#0072B2","#D55E00" ,"#009E73" ,"gray")
...
ggplot(elevation_df, aes(x = longitude, y = latitude, fill = elevation)) +
  geom_tile() +
        scale_fill_gradientn(colors = c("darkolivegreen3","saddlebrown" ,"white"))+
  theme_minimal() +
  coord_quickmap() +
...
ggsave("3_Figures/FigX_Western_US_elevation.png", height=10, width=11)

```


trying to add state outlines and boxes for the sites
```{r}
# Step 1: Load state boundary data (already loaded in previous code)
all_states <- st_read("2_DataAnalysis/cb_2018_us_state_5m/cb_2018_us_state_5m.shp")

# Step 2: Subset for Western U.S. states, including Nevada
west <- subset(all_states, NAME %in% c("Wyoming", "Colorado", "Idaho", "Utah", 
                                       "Montana", "California", "Washington", 
                                       "Oregon", "Arizona", "New Mexico", "Nevada"))
...
# Step 13: Plot the masked elevation data with state boundaries on top
elevation_df <- as.data.frame(raster::rasterToPoints(elevation_subset), stringsAsFactors = FALSE)
colnames(elevation_df) <- c("longitude", "latitude", "elevation")

west_df <- as.data.frame(raster::rasterToPoints(west_sp), stringsAsFactors = FALSE)
colnames(west_df) <- c("longitude", "latitude", "elevation")
...
ggplot(elevation_df, aes(x = longitude, y = latitude, fill = elevation)) +
  ...
  theme_minimal() +
  coord_quickmap() +
  labs(title = "Elevation in the Western U.S.", fill = "Elevation (m)") +
  # Add state boundaries on top
  geom_sf(data = west_sp, fill = NA, color = "black", size = 0.5) +
  # Add a box around Mount Rainier (approximate coordinates)
  annotate("rect", xmin = -122.0, xmax = -121.5, ymin = 46.7, ymax = 47.0, 
           color = "red", fill = NA, size = 1) +
  # Add text label for Mount Rainier (optional)
  annotate("text", x = -121.75, y = 46.85, label = "Mount Rainier", color = "red", size = 4, hjust = 0.5)

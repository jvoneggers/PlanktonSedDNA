...
asv_melt <- taxa %>% rename(domain = Kingdom, supergroup=Groups,class=Class,order=Order,family=Family,genus=Genus,species=Species ) %>%  separate(Phylum, into = c("division", "subdivision"), sep = "-") %>% select(asv_code, domain, supergroup, division, subdivision, class, order, family, genus, species)

# Import pre-formatted FUNGuild database (downloaded 2023/04/21)
funguild <- read_tsv("funguild_db.tsv", col_names = TRUE) %>%
...
# Write melted ASV and function data to file
 ...
 asv_melt_function %>%
   write.csv("2_DataAnalysis/V7_Taxonomy_Function.csv")
 ...
 source("1_LoadData/1_CreatePhyloseq_ZOTUexact_TAX80.R")
 taxa<-as.data.frame(tax_table(ps_V9_norm))
 taxa$asv_code<-row.names(taxa)
 asv_melt <- taxa %>% rename(domain = Kingdom, supergroup=Groups,class=Class,order=Order,family=Family,genus=Genus,species=Species ) %>%  separate(Phylum, into = c("division", "subdivision"), sep = "-") %>% select(asv_code, domain, supergroup, division, subdivision, class, order, family, genus, species)
 
 # Import pre-formatted FUNGuild database (downloaded 2023/04/21)
 funguild <- read_tsv("2_DataAnalysis/funguild_db.tsv", col_names = TRUE) %>%
         dplyr::select(taxon, trophicMode, guild) %>%
         filter(str_count(taxon, " ") <= 1) %>%
         distinct() %>%
         arrange(taxon)
 ...
 asv_melt_function %>%
         write.csv("2_DataAnalysis/V9_Taxonomy_Function.csv")
